...
**Imagery shows real things.** A picture on a Mnemos page is a picture of something that exists: a frame of the world drawn by its own engine, a capture of a screen that runs, a page a mind actually drew. The display can carry it — an image drawn onto the lattice as charge, resolving to the true image when the reader arrives at it — or housing can frame it on a hairline. No stock, no illustration standing in for the thing itself, no image of something the build cannot currently show.
