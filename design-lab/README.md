...
- `panes-and-widgets.html` (this folder) — the shell study for widgets, the
  pane system (split · resize · tear off), and the agent's place. Real
  identity marks, generated by `glyph.ts` and embedded. Scenes via query:
  `?theme=light`, `?place=1`, `?float=1`, `?laws=1` (the laws overlay names
  which surfaces are frame / content / lifted). Design brief:
  `docs/luca/EXTENSIONS.md`.
