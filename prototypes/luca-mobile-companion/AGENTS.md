...
## Luca Companion Direction

- Do not generate additional concept images for this prototype. Iterate in the working frontend.
- The earlier transcript-and-drawer prototype was rejected as sloppy, incomplete, and insufficiently specific to Luca Agent Network. Do not restore it as the default shell.
- The current direction is an ambient intelligence surface organized as two places and one contextual event: Network and Room are spatial places; Permission unfolds over the Room from the requesting resident mark. Do not restore a third destination, persistent mode rail, or action tab.
- The room is the primary surface and a familiar conversation system: a scrolling multi-turn transcript with a persistent text composer (ChatGPT-like interaction model). Visual design stays Luca-native — identity marks, near-black/paper palette, no iMessage bubbles, no glass dock, no mode rail.
- Motion follows one rule: places transition, events unfold, and a resident mark is the only object that may work, travel, or ask. Horizontal movement is reserved for Network/Room place changes; Permission expands from and recedes into the requesting mark. Never use bounce for personality.
- The supplied Polyphonic mobile screenshots inform the OLED-black palette, restrained typography, spacious bubble-free transcript, hamburger navigation, compact conversation selector, and tall composer anatomy. They do not define Luca's product architecture.
- Keep the experience Luca-native: direct conversations with persistent residents, multi-resident rooms, Mac availability, activity, permissions, and stable identity marks.
- Do not introduce Polyphonic-specific or invented concepts such as Trace, model catalogs, mind dashboards, launch artifacts, or website-oriented controls.
- Keep the phone a companion conversation surface for residents hosted by the user's Mac; do not imply that residents run locally on the phone.
- Keep the Room conversation-first and visually quiet. Persistent composer: text field + send always available. The paper orb is voice-only: hold for 180ms to speak, release to send. Short orb tap focuses the text field.
- Permission is always Room-owned and per-request. Keep the Room mounted beneath it, preserve clearly labeled Approve and Decline exits, and return directly to a consequence statement rather than a result page.
- Keep Network and Room as the only spatial places even as the prototype grows into the full companion frontend. Conversations, activity, notifications, privacy, devices, and settings are records presented from the Network; they are not peer places and must not restore a tab bar or mode rail.
- The full frontend covers pairing/recovery, conversations, resident detail, owner-visible activity, exact permission decisions, connection state, notification privacy, and this phone's settings/device removal. It does not expose runtime configuration, provider credentials, Brain administration, filesystem paths, or mobile-hosted residents.
- The prototype-only deterministic states are `?scene=network|room|permission|approved|declined|onboarding|pairing|conversations|activity|notifications|settings|privacy|devices|offline`. They exist for QA and captures, not as production routes.
