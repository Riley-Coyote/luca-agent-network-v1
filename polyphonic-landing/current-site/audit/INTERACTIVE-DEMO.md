...
## The demo is the app — 2026-09-09 (WP-03)

Aligned every simulated screen to the real desktop app's structure, labels and rhythm, keeping the fictional Northstar fixtures and every interaction.

- **Rail.** `PROJECTS` (`#` Northstar, `+`) · `MESSAGES` (Luca now, Mira unread, Ziggy 2d, Luca & Mira 1w, `+`) · `RUNTIMES` (six marks, green dots) over the "Y / You / Personal workspace" footer. The static no-JS rail in `index.html` uses the same labels.
- **Agents.** The app's three columns: a list column ("Agents", "7 agents", `+`, "Search agents", chips All · Running · Attention, a quiet "6 runtimes connected" strip carrying all six marks, then seven personal agents — Luca, Mira, Ziggy, Iris, Wren, Otto, Nia — each with an initials glyph, a `Runtime · Status · note` line and its runtime's mark; footer Groups · Defaults) beside a detail column (glyph, name, status dot, `Runtime · demo · <key>`, Message pill and four icon actions, tabs Documents · Notebook · Settings, the six resident files with "You write this" / "<Agent> writes this", two edited and four "Not written yet", then `MORE FILES`, "+ New file", "Nothing else in the folder yet." and the `residents/<key>` path).
- **Brain.** The app's Connections screen: Repositories `Found` (3 found · 3 items available), Codex `Current` (1 connected · 412 searchable sections), Claude Code `Current` (1 connected · 638 searchable sections), Files `Current` (2 files · Northstar brief.md, How I like to work); tabs Connections · Activity, "Scan again", "Private and local". The four Northstar grants the page's own "Choose what Luca can see" section relies on are outside the frame and untouched; source documents are still opened from Library and from the two Details buttons.
- **Activity.** Eyebrow `YOUR AGENTS AT WORK`, the two live runs (Iris on Welcome screen with Claude Code, Wren on Project sync with Codex) keeping the existing progress meters and cross-window sync, then `RECENT` cards for Luca, Mira, Ziggy, Otto and Nia with `READY` / `STARTED` / `STOPPED` and Open / No accessible conversation.
- **Project room and drawer.** `PROJECT` header block with the `+` and room ages, footer Sources · Project details; room conversations open with a date divider, "You created this channel" and "<Agent> was added by You, along with <Agent>", and close with a Between agents receipt. The conversation drawer is the app's stack: DIRECT MESSAGE · AT A GLANCE (Present 2 · Agents 1 · Files 4 · Exchanges 3) · AGENTS · PEOPLE · WORKING CONTEXT · BETWEEN AGENTS, with "Manage participants" in the footer. Its member tabs still filter the exchange.
- **Unchanged.** Navigation history, split, drawer, pop-out (in-page and separate window), expanded view with its focus trap, guided tour, draft isolation, runtime selection in Settings, progress simulation, pause and reduced motion. Colours and type inside the frame are still WP-02c's; new elements read only the tokens pinned on `.app-frame` / `.d-popout`.

Verified on a fresh build: `verify-demo` and `verify-release` pass; `verify` fails only on its pre-existing no-JS hero assertion (`Your agents,\nTOGETHER`), which WP-02 changed and which is unrelated to this pass. Axe reports the same single pre-existing page-footer contrast violation as before and no new ones. All twenty simulated views open with the right title, zero text nodes inside the frame compute to pure white, no horizontal overflow at 1440 / 1024 / 390 / 320, the frame's outer geometry is identical to the WP-02c build at all three widths, and the page outside the frame is pixel-identical to it (mean absolute difference 0.0 over 6.08 M pixels). Console clean. Three fixture assertions in `scripts/verify-demo.mjs` were updated, each commented `WP-03`, because this brief deliberately replaced what they asserted.
