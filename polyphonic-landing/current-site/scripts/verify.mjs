...
/* WP-06: the seven-window band replaced card 1. Seven windows plus an aria-hidden duplicate; under
   reduced motion (emulated above) it must not animate and must be a keyboard-reachable scroller. */
const band=await page.evaluate(()=>{const b=document.querySelector('.pw-band'),t=document.querySelector('.pw-track');return{all:document.querySelectorAll('.pw-window').length,dup:document.querySelectorAll('.pw-half-dup .pw-window').length,dupHidden:document.querySelector('.pw-half-dup').getAttribute('aria-hidden'),anim:getComputedStyle(t).animationName,tabindex:b.getAttribute('tabindex'),scrolls:b.scrollWidth>b.clientWidth,names:[...document.querySelectorAll('.pw-half:not(.pw-half-dup) .pw-name')].map(e=>e.textContent),runtimes:[...document.querySelectorAll('.pw-half:not(.pw-half-dup) .pw-runtime')].map(e=>e.textContent)}});
...
/* WP-13: generic label words are written lowercase in the HTML; names (Mac, Mnemos) keep their case. */
...
assert.equal(narrative.horEyebrow,'next · the network'); /* WP-13: lowercase label */
...
assert.equal(narrative.betaEcho,'first room of something larger.'); /* WP-13: lowercase label */
...
const nojs=await b.newPage({javaScriptEnabled:false,viewport:{width:390,height:844}});await nojs.goto('http://127.0.0.1:8744/');/* WP-13: LIVE is the one Doto word (.dt is uppercase, and innerText reports rendered case); the full stop stays in the sans. */assert.equal(await nojs.locator('h1').innerText(),'Give them\nsomewhere to LIVE.');assert.ok((await nojs.locator('.chat-transcript').innerText()).includes('Codex finished'));out.checks.push('Static hero and complete conversation readable without JavaScript');await b.close();
