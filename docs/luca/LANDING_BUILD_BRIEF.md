...
- The notch design exists: `design-artifacts/kao/` (`kao.html` study b, and `luca-demo.html` for the orchestration-in-the-notch loop). Build from it — phosphor eyes asleep as two dim dots, the notch *opening* as the door, progress rings for dispatched agents, a one-line drawer beneath. Apply the direction notes in that folder's README (no amber, cool ink, near-black notch). Show the consent beat — eyes wake → go cold (cyan, "reading your screen") → only then the rings — without explaining it. Context: Prompt Ghost (`~/Documents/Repositories/prompt-ghost`, a native macOS ambient agent layer that already connects to Luca) is merging into this presence — the notch is the face, the hotkey/composer the door, Ghost Light's razor-thin hairline the attention. Its glass-panel language does NOT carry over; phosphor + hairline on near-black does.
