...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Luca."
        : "Updates workflow state in Luca.",
  ...
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads identity or presence data."
        : "Updates identity or membership data.",
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Luca." : "Writes to Luca.",
    tone: isWrite ? "write" : "read",
  };
}
