...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a Luca event, there is no event content to surface, so
  // userText is empty and the title falls through to "Luca event".
  ...
  assert.equal(result.userTitle, "Luca event");
...
test("parsePromptText defaults the title to 'Luca event' when no kind is present", () => {
