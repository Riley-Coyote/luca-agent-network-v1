...
function isEmptySharedComputeError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("no shared compute serving members") ||
    normalized.includes("no live shared compute models") ||
...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "Luca is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of Luca cannot use shared compute. Update Luca or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "Luca received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "Luca couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }
