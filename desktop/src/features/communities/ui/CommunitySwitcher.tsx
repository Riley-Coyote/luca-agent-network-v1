    ...
    <>
      ...
      {variant === "profile-menu" ? (
        ...
        <ChevronDown
          className={
            isProfileVariant
              ? "h-4 w-4 shrink-0 text-ink-faint"
              : "h-4 w-4 shrink-0 text-ink-faint"
          }
        />
      )}
      ...
      <DropdownMenuTrigger asChild>
        {variant === "profile" ? (
          <button
            ...
            className="flex min-w-0 max-w-full items-center gap-1.5 rounded-md py-0.5 text-left text-xs text-ink-faint outline-hidden transition-colors hover:text-sidebar-foreground focus:outline-none focus-visible:outline-none data-[state=open]:text-sidebar-foreground"
            data-testid="community-switcher"
            type="button"
          >
            {triggerContent}
