...
export function LucaCollectionPanel({
  ...
  const collectionChats = channels
    .filter((channel) => {
      ...
      return [...channel.memberPubkeys, ...channel.participantPubkeys].some(
  ...
  return (
    <aside
      className={cn(
        "flex w-72 shrink-0 flex-col border-r border-border/55 bg-sidebar/72",
        "max-md:absolute max-md:inset-y-0 max-md:left-0 max-md:z-40 max-md:w-full max-md:bg-sidebar max-md:pt-8",
        selectedChannelId && "max-md:hidden",
      )}
