...
const THREAD_REPLY_ANCHOR_SIZE_REM = 1; // Tailwind w-4
...
const THREAD_REPLY_ANCHOR_RADIUS_REM = THREAD_REPLY_ANCHOR_SIZE_REM / 2;
const THREAD_REPLY_ANCHOR_LINE_GAP_REM = 0.25; // Tailwind spacing-1

export const THREAD_REPLY_BODY_OFFSET_REM =
  THREAD_REPLY_ROW_MARGIN_INLINE_REM +
  THREAD_REPLY_ROW_CONTENT_INSET_REM +
  THREAD_REPLY_ANCHOR_SIZE_REM +
...
const THREAD_REPLY_ANCHOR_CENTER_OFFSET_REM =
  THREAD_REPLY_ROW_MARGIN_INLINE_REM +
  THREAD_REPLY_ROW_CONTENT_INSET_REM +
  THREAD_REPLY_ANCHOR_SIZE_REM / 2;
const THREAD_REPLY_ANCHOR_CENTER_Y_REM =
  THREAD_REPLY_ROW_PADDING_TOP_REM + THREAD_REPLY_ANCHOR_SIZE_REM / 2;
...
export function getThreadReplyAnchorCenterRem(depth: number) {
  return getThreadReplyIndentRem(depth) + THREAD_REPLY_ANCHOR_CENTER_OFFSET_REM;
}

function getThreadReplyAnchorCenterForVisibleDepthRem(visibleDepth: number) {
  return (
    getThreadReplyIndentForVisibleDepthRem(visibleDepth) +
    THREAD_REPLY_ANCHOR_CENTER_OFFSET_REM
  );
}

export function getThreadReplyAnchorCenterYRem() {
  return THREAD_REPLY_ANCHOR_CENTER_Y_REM;
}

export function getThreadReplyDescendantRailStartYRem() {
  return (
    THREAD_REPLY_ANCHOR_CENTER_Y_REM +
    THREAD_REPLY_ANCHOR_RADIUS_REM +
    THREAD_REPLY_ANCHOR_LINE_GAP_REM
  );
}
...
export function getThreadReplyConnectorLayout(depth: number) {
  ...
  const parentOffsetRem = getThreadReplyAnchorCenterForVisibleDepthRem(
    visibleDepth - 1,
  );
  const childOffsetRem =
    getThreadReplyAnchorCenterForVisibleDepthRem(visibleDepth);
  ...
  return {
    childOffsetRem,
    heightRem: THREAD_REPLY_ANCHOR_CENTER_Y_REM,
    parentOffsetRem,
