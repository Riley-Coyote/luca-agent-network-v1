...
use luca_continuity::{
    derive_revision_idempotency_key, encrypt_record, encrypted_record_reference, RecordMetadata,
    RevisionActor, RevisionOperation, RevisionRequest,
};
use luca_protocol::{
...
fn ready_runtime(temp: &TempDir) -> (Arc<Mutex<ContinuityRuntimeState>>, NamespaceScope) {
    let mut store = match ContinuityStore::open(temp.path(), ContinuityStoreCustody::Ready).unwrap()
    {
...
fn owner_forget_physically_purges_every_encrypted_handoff_revision() {
    ...
    let outcome =
        forget_resident_handoff_locked(&runtime, &hex('1'), &hex('2'), &id("owner-forget-test"));
...
fn boot_existing_database_never_acquires_while_new_path_may_initialize() {
    ...
    let store =
        match ContinuityStore::open(existing_temp.path(), ContinuityStoreCustody::Ready).unwrap() {
            ContinuityStoreOpen::Ready(store) => store,
            ContinuityStoreOpen::Degraded(_) => panic!("expected ready store"),
        };
    drop(store);
