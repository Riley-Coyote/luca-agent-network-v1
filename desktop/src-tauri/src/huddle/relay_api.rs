...
pub(crate) async fn connect_audio_relay(
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            luca_log!(info, "buzz-desktop: audio relay pipeline exited: {e}");
        }
        ...
        loop {
            ...
            for chunk in samples.chunks(FRAME_SAMPLES) {
                ...
                let n = match encode_result {
                    Ok(n) => n,
                    Err(e) => {
                        luca_log!(warn, "buzz-desktop: opus encode error: {e}");
                        continue;
                    }
                };
                if n > 0 {
...
pub(crate) async fn fetch_channel_members_with_roles(
    ...
    let events = query_relay(state, std::slice::from_ref(&filter))
        .await
        .map_err(|e| {
            luca_log!(warn, "buzz-desktop: fetch channel members failed: {e}");
            e
        })?;
