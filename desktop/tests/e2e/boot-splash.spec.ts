...
// loading gate keeps a quiet application-canvas overlay above the already
...
test("boot splash overlay holds on the application canvas, then dismisses", async ({
  page,
}) => {
  ...
  // Cold boot remains visually quiet and never restores the legacy Buzz
  // spectrum layer.
  const loadingGate = overlay.getByTestId("app-loading-gate");
  await expect(loadingGate).toBeVisible();
  await expect(loadingGate).toHaveCSS("background-image", "none");
  await expect(overlay.getByTestId("setup-grainient-background")).toHaveCount(
    0,
  );

  // The app mounts and loads beneath the overlay — boot is not delayed.
  const appChrome = page.getByTestId("app-top-chrome");
  await expect(appChrome).toBeVisible();
